...
public class SchedulingController extends BaseController
{
    ...
    public AjaxResult addScheduling(@RequestBody HashMap<String, Object> data) throws ParseException {
        ...
        int count = videoShopSchedulingService.selectCountByTimeBetween(Long.valueOf((String) data.get("roomId")),Long.valueOf((String) data.get("anchorId")),(String) data.get("timeBegin"),(String) data.get("timeEnd"));
        ...
        while (calendar.getTime().before(timeEnd) || calendar.getTime().equals(timeEnd)){
            ...
            if(data.get("roomId")!=null){
                VideoShopLiveRoom videoShopLiveRoom = videoShopLiveRoomService.selectLiveRoomByRoomId((String)data.get("roomId"));
                videoShopScheduling.setRoomId(Integer.valueOf((String)data.get("roomId")));
                videoShopScheduling.setRoomName(videoShopLiveRoom.getRoomName());
            ...
            videoShopSchedulingService.insertVideoShopScheduling_UP(videoShopScheduling);
        }
    ...
    public AjaxResult updateScheduling(@RequestBody HashMap<String, Object> data) throws ParseException {
        ...
        int count = videoShopSchedulingService.selectCountByTimeBetweenEdit(Long.valueOf((String) data.get("roomId")),Long.valueOf((String) data.get("anchorId")),(String) data.get("timeBegin"),(String) data.get("timeEnd"));
        if(count>0){
            return AjaxResult.error("该时间段已存在直播排班，请重新设置");
        }
...
//    @RequiresPermissions("system:scheduling:export")
    ...
    @PostMapping("/export/{roomId}")
    @ResponseBody
    public AjaxResult export(VideoShopScheduling videoShopScheduling,@PathVariable("roomId") Integer roomId)
    {
        videoShopScheduling.setRoomId(Integer.valueOf(roomId));
